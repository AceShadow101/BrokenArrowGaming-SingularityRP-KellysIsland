...
_fine = 1;
_totalprobation = 5;
life_gear = getUnitLoadout player;
   ...
   _fuck = 120;
   	while{_fuck > 0} do {
	   	...
	   	if(_fuck == 1) exitwith {
		   	...
		   	["You have been fined $200 for not picking up rubbish", false] spawn domsg;
			if(_fine == 3) then {
				["You have been added to the wanted list for skipping jail work & fined $5000", false] spawn domsg;
				[getPlayerUID player,profileName,"901A"] remoteExec ["life_fnc_wantedAdd",2];
...
removeAllWeapons player;
player setUnitLoadout life_gear;