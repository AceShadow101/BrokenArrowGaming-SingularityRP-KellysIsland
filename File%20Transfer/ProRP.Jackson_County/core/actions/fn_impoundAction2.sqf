...
// Testing - TheOnlyLupo
if ((player distance (getMarkerPos "impoundlot") > 25) && !license_civ_protowing) exitWith {};
// End Test
...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if((count crew _vehicle) == 0) then
	{
		...
		switch (true) do
		{
			case (_vehicle isKindOf "Car"): {_price = 5000 + round(random 3500);};
			case (_vehicle isKindOf "Ship"): {_price = 7500 + round(random 3500);};
			case (_vehicle isKindOf "Air"): {_price = 12500 + round(random 3500);};
		};
		...
		[[0,format[localize "STR_NOTF_Hasprotowing",profileName,(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
