...
if (vehicle player != player) then {hint "You Cannot Process From Your Vehicle!"};
if (life_is_processing) exitWith {hint "You're Already Processing Something!"};
if (vehicle player != player) exitwith {hint format["You Must Exit Your Vehicle!"]};
...
if (!_lizenzBoolean) then {
	if (life_cash>=_lizenzKaufkosten) then {
		...
		hint format ["You Bought a License for %1 Dollars!",_lizenzKaufkosten];
	} else {
		hint format ["You Do Not Have The Funds For This License!",_lizenzKaufkosten];
...
if (_nah) exitWith {life_is_processing = false;5 cutText ["","PLAIN"];hint "You Do Not Have The Necessary Items!"};
...
while{true} do
{
	...
	if(_cP >= 1) exitWith {};
	if(player distance _currentPos > 10) exitWith {hint "You Are Too Far Away!";life_is_processing = false;5 cutText ["","PLAIN"];_nah=true};
};
if (_nah) exitWith {life_is_processing = false;5 cutText ["","PLAIN"]};
