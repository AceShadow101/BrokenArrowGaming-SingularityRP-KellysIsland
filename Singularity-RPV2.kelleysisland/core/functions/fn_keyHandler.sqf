...
switch (_code) do {
	//Space key for Placeables (by Maximum) and Jumping 
case 57: {
		[] spawn life_fnc_placeablesPlaceComplete; // <<-- ADD THIS LINE
		[] spawn life_fnc_placeablesPlaceCompleteMEDIC;
	...
	case 38: {
		...
		if(_shift && playerSide in [west,independent]) then {
			if(vehicle player != player && (typeOf vehicle player) in ["C_Offroad_01_F","I_MRAP_01_F","C_SUV_01_F"]) then {
	...
	//Num Pad+N = Police Panic Button
	case 49:{	
	    if (_shift && !_alt && !_ctrlKey) then
		{  
		if (playerSide != west) exitWith{};
		[[1,format[localize "STR_NOTF_Panic",profileName, mapGridPosition player]],"life_fnc_broadcast",west,FALSE] call life_fnc_MP;
		_handled = true;
